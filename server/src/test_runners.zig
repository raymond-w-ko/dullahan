...
fn runImageTest(allocator: std.mem.Allocator, args: ?[]const u8) !void {
    ...
    try writeAllFd(stdout_fd,
        ...
        \\=========================
        \\
        \\RGB raw f=24. Expected: opaque 20x8 cell gradient.
        \\
    );
    ...
    try writeAllFd(stdout_fd,
        \\RGBA raw f=32. Expected: checker transparency over terminal background.
        \\
    );
    try writeRawImage(allocator, stdout_fd, rgba_data, .{
        .format = .rgba,
        .width = image_width,
        .height = image_height,
        .image_id = 2,
    ...
    try writeAllFd(stdout_fd,
        \\PNG opaque f=100. Expected: opaque red sample scaled to 20x8 cells.
        \\
    );
    try writeEncodedImage(stdout_fd, opaque_png_b64, .{
        .format = .png,
        .image_id = 3,
        .placement_id = 3,
        .cols = cols,
    ...
    try writeAllFd(stdout_fd,
        \\PNG alpha f=100. Expected: translucent red sample blending with background.
        \\
    );
